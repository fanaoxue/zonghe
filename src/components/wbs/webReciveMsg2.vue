<template>
    <div class="web-recive-msg2">
        <c-header>
            <x-header :left-options="{ backText: '' }">收款人信息</x-header>
        </c-header>
        <group label-width="7.0em" label-margin-right="0.2em" label-align="right">
            <cell title="收款人姓名：" v-model="driverName" value-align="left"></cell>
            <cell title="收款人身份证：" v-model="idCard" value-align="left"></cell>
            <cell title="手机号码：" v-model="mobileNo" value-align="left"></cell>
            <cell title="支付宝账号：" v-model="alipayNo" value-align="left"></cell>
            <!--<x-input
                title="支付宝账号："
                v-model.trim="alipayNo"
            ...
            <div class="vux-x-input weui-cell cell-before" v-show="buttonShow">
        ...
        <!--<div class="footer" v-show="buttonShow">
...
export default {
    ...
    data() {
        return {
            ...
            cartBadgeNo: '', //车牌号
            alipayNo: this.$route.query.alipayNo, //支付宝账号
            idCard: this.$route.query.idCard, //司机身份证
            bankNum: '', //银行卡号
            bankName: [], //银行名称
    ...
    //         }
    //     })
    // }else if(from.name == 'alipayQRcode'){
    //   next((v) => {
    //     //进页面的时候，返回存储的数据
    ...
    created() {
        ...
        this.alipayNo = this.alipayNo;
    },
    mounted() {
        // this.$nextTick(() => {
        //     this.$store.commit('updateLoadingStatus', {isLoading: false})
        //     getHkbBankList().then((res) => {  // 9002
        //         this.$store.commit('updateLoadingStatus', {isLoading: false})
                        ...
                        .then(res => {
                            ...
                            if (res.data.reCode === '0') {
                                _this.$vux1.confirm.show({
                                    ...
                                    title: '提示',
                                    content: '绑定成功,恭喜您获得10元奖励金可在【个人中心-回款宝推荐奖励】中查看',
                                    confirmText: '知道了',
                                ...
                                //5,6,7,8,9,10
                                this.$vux.alert.show({ title: '提示', content: res.data.reInfo });
                            }
                        })
                        .catch(err => {
                            this.$store.commit('updateLoadingStatus', { isLoading: false });
